...
export default function AdminVuxTab() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      await fetch(`${VUX_API_URL.replace(/\/+$/, "")}/api/ping`).then((res) => {
      ...
      // 2. Speed
      const speedRes = await fetch(`${VUX_API_URL.replace(/\/+$/, "")}/api/speed`);
      if (speedRes.ok) {
      ...
      // 3. Health
      const healthRes = await fetch(`${VUX_API_URL.replace(/\/+$/, "")}/api/health`);
      if (healthRes.ok) {
